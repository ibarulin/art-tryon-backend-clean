...
function absolutize(possibleUrl, base) {
  try { return new URL(possibleUrl).toString(); }
  catch { return new URL(possibleUrl, new URL(base)).toString(); }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Тянем HTML страницы с дружественными заголовками
    ...
    // 3) JSON-LD блоки: "image": "...", или "image": ["..."]
    if (!imageUrl) {
      ...
      for (const m of ldBlocks) {
        try {
          ...
          if (Array.isArray(json)) json.forEach(obj => pushImage(obj?.image));
            ...
            // Иногда в offers/image или in "itemListElement"
